...
Run by the commit-baselines job (issues 038 + 053) and by nothing else — it is
...
def main():
    ...
    if not added and not verdict.get('changes'):
        if os.path.exists(LOG_JSON) and os.path.exists(LOG_MD):
            print('nothing moved — baselines already current, no log entry written')
            return 0
        # First run under this policy with nothing to record: initialise the
        # (empty) logs anyway, so the user guide's and llms.txt's links to them
        # resolve from day one — an honest "nothing has moved yet", not a 404.
        ...
        print('log initialised — nothing has moved yet')
        return 0
