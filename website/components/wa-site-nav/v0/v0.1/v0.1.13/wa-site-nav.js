/* wa-site-nav v0.1.13 — THE site header (issues 048 + 054). One source for every
   page.

   v0.1.12 sends the Library menu to PAGES (Dinis, from QA: "how it works now
   links to the home page, and it should go to the page we auto update on CI").

   Three of its five entries were home-page fragments — /#how, /#privacy,
   /#sources. They were honest when the home page was mostly prose, but a menu
   called Library sitting under a hamburger promises documents, and what it
   delivered was a scroll on the page you were already looking at. Worse, the
   thing "How it works" should reach already existed and is rebuilt by CI on
   every deploy: /user-guide/, whose screenshots are captured by the qa-to-docs
   journeys of that same run, so it cannot describe a build that never shipped.

   Each fragment is now a page that owns the subject:
     How it works        → /user-guide/          (CI-rendered from passing tests)
     How the browser works → /engineering/wasm/  (new — FFmpeg as WebAssembly)
     Where your audio goes → /engineering/security/
     Getting the audio   → /tools/extract-audio/

   The home page keeps every one of those ids: they are published, they are
   linked from the key guide, and #privacy is still where the routing statement
   lives. What changed is where the MENU points.
...
/* Every entry is a page, and every page is the best answer to its own label.
   "How it works" is the user guide because that is the page CI rebuilds each
   deploy from screenshots a passing test took of THAT build — the one document
   here that cannot drift from the product. */
const LIBRARY = [
    ['/user-guide/',           'How it works — the user guide'],
    ['/openrouter-key/',       'Getting a key'],
    ['/engineering/wasm/',     'How the browser does the work'],
    ['/engineering/security/', 'Where your audio goes'],
    ['/tools/extract-audio/',  'Getting the audio out of a video'],
]
/* The home page still carries #how, #privacy and #sources — they are published
   addresses and the key guide links them. A gate in
   ...
   200 and so never shows up as one. It now also asserts that a menu entry
   pointing at a PAGE resolves to a file that exists. */
