/* wa-debug-panel v0.1.0 — the advanced/debug pane (issue 027).
...
class WaDebugPanel extends SgComponent {
    ...
    setOpen(open) {
        ...
        if (open) { this.renderExchanges(); this.renderPrompts() }
    }
    ...
    exchangeRow(e, wasOpen) {
        ...
        s.innerHTML = `${badge} ${status} <span class="meta">${bits.join(' · ')}</span>`
        d.append(s)
