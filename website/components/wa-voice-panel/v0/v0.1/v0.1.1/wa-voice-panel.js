/* wa-voice-panel v0.1.1 — the Updates, read aloud, on demand (issue 062).

   v0.1.1 carries the rename only: the spoken lead-in said "Voice Note
   Transcribe", which is now VoiceDebrief (issue 060). A new version rather than
   an edit because v0.1.0 is published, and a published path is immutable — this
   one is read out loud, so the old name would have gone on being spoken.
...
export function draftScript (post) {
    if (!post) return ''
    const lead = 'Here is the latest from VoiceDebrief.'
