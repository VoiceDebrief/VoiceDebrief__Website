/* wa-locale-picker v0.1.4 — the panel actually closes.

   THE BUG THIS FIXES, because it is the instructive kind. Every version from
   v0.1.1 rendered the culture list on page load, on every viewport, while
   reporting itself closed. `toggle()` sets `panel.hidden`, which closes an
   element only via the user-agent rule [hidden]{display:none} — and the .panel
   rule sets display:grid, which is an AUTHOR rule and therefore wins. The
   attribute was set, the property read back true, and 612x328 pixels of panel
   sat on the page regardless.

   It survived review twice because the check was `panel.hidden` — the property
   faithfully agreed with the code that set it, and neither had anything to do
   with what a browser painted. The fix is one line (.panel[hidden]{display:none});
   the lesson is that visibility is only ever verified by computed style or by a
   box, never by the attribute that was supposed to cause it.

   v0.1.3 anchored the panel to the SCREEN on phones.
...
<style>
  ...
  /* THIS RULE IS LOAD-BEARING. The hidden attribute closes an element only
     because the user-agent stylesheet says [hidden]{display:none} — and ANY
     author rule setting display beats the UA sheet. The .panel rule below sets
     display:grid, so from v0.1.1 until now the panel rendered on every page
     load with hidden="" faithfully set on it. Setting .hidden in JS looked
     correct, read back correct, and did nothing. Never control this element's
     visibility from the attribute alone. */
