/* vd-workflow v0.1.0 — the panel from the design pack (issue 060, M3).
   ...
   Drive it:
     ...
     el.startRun([{id,label}, …])     → running
     el.stepDone(id) / el.stepFail(id)
...
.steps li[data-status=failed] .mark{border-color:var(--vd-e,#C0392B);color:var(--vd-e,#C0392B)}
...
class VdWorkflow extends HTMLElement {
    ...
    startRun(steps) { this._steps = steps.map(s => ({ ...s, status: 'todo' })); this._go('running') }
    stepDone(id) { this._mark(id, 'done') }
    stepFail(id) { this._mark(id, 'failed') }
    showResults(r) { this._results = r; this._go('results') }
    showError(e) { this._error = e; this._go('error') }
    reset() { this._file = null; this._results = null; this._error = null; this._go('empty') }
    ...
    _mark(id, status) {
        ...
        if (!s) return
        s.status = status
        const next = this._steps.find(x => x.status === 'todo')
        if (next && status === 'done') next.status = 'now'
    ...
    render() {
        ...
        // The routing statement shows in every state that still has a file to
        // send. In results it is replaced by the caveat, which qualifies a thing
        // that now exists.
        const foot = this._state === 'results'
...
<ul class="steps" aria-live="polite">${steps.map(s => `
  <li data-status="${s.status}"><span class="mark">${mark[s.status]}</span>${esc(s.label)}</li>`).join('')}</ul>
<p class="billing">Nothing is stored. Close the tab and it is gone.</p>`
    }
    ...
    _resultsHtml() {
        ...
        const panes = [
            ...
            ['infographic', 'Infographic', r.infographicHtml],
        ].filter(([, , v]) => v)
        const active = this._tab && panes.some(p => p[0] === this._tab) ? this._tab : (panes[0]?.[0] || '')
        return `<div class="tabs" role="tablist">${panes.map(([id, label]) => `
  <button type="button" role="tab" data-tab="${id}" aria-selected="${id === active}">${label}</button>`).join('')}</div>
${panes.map(([id, , v]) => `<div class="pane" data-pane="${id}"${id === active ? '' : ' hidden'}>${
    id === 'infographic' ? v : md(v)}</div>`).join('')}
<div class="resfoot">
