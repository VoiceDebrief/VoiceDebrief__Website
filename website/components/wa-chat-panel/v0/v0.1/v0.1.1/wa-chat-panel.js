/* wa-chat-panel v0.1.1 — talk to your materials (issue 034, M3).
   v0.1.1 (issue 035): suggestion chips only show on an empty thread (they were
   eating screen space mid-conversation); tool rows can mark an attached image
   (view_infographic); tier badge for the material-editing tools.
...
class WaChatPanel extends SgComponent {
    ...
    async renderThread() {
        ...
        this.sugg.hidden = h.messages.length > 0   // suggestions are for a blank page only
    ...
    messageEl(m) {
        ...
        if (m.role === 'tool') {
            ...
            div.innerHTML = `<span class="did">⚙ ${this.esc(m.did)}</span> ${this.esc(m.content)}` +
                (m.image ? ' <span class="img-mark">🖼 image attached — the model can see it</span>' : '')
            return div
        }
