...
- **See it work without a key, and a setup step that finishes (issue 062, Dinis)** —
...
- **llms.txt gets a public explanation, fresh links, and a rulebook clause
  ...
  surfaces (user guide + `guide.json`, the browser test runner, versions and
  videos pages, the screenshot change log); `record_baseline_changes.py` now
...
- **The user guide now writes itself from the tests (issue 038, M-qtd-2)** — a new
  ...
  that was, in that same run, asserting the product works, so the guide cannot quietly
  drift from the product. Three sections, captions from the manifest, and an honest
  "not captured yet" placeholder for any shot without a baseline.
  - **CI now arms the baselines** (`commit-baselines`, qa only). Previously the
    captures were uploaded as an artifact and died with the container, and the
    harness's own "commit it to arm the gate" instruction had no mechanism behind
    it — the gate could never arm and the guide had no pictures. The job commits
    **only shots with no existing baseline**; a shot whose baseline exists and
    differs is the gate firing, so the test job is already red and nothing
    overwrites the evidence. Auto-blessing a diff would turn the gate into a
    recorder that files every regression as the new truth.
  - **No CI loop**: the push uses `GITHUB_TOKEN`, whose pushes do not trigger
    workflows — the same property the version bump on `dev`/`main` already relies
    on — with `paths-ignore` on the screenshots path as a second guard. The deploy
    job pulls the new commit so the shots ship in the same run.
  - **Shots are now framed** (`clip` in `journeys.json`): the first screen, a
