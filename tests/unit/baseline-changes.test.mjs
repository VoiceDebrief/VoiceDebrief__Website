...
test('a run that moved nothing writes no entry — but the first one initialises the logs', () => {
    const saved = snapshot()
    try {
        // First quiet run with no logs on disk: initialise them (empty), so the
        // user guide's and llms.txt's links resolve from day one, never 404.
        const first = record(captures([]))
        assert.match(first, /log initialised|nothing moved/)
        assert.ok(existsSync(logJson) && existsSync(logMd), 'both logs exist after the first run')
        assert.equal(JSON.parse(readFileSync(logJson, 'utf8')).entries.length, 0, 'and hold no entries')
        // Every later quiet run: leave them alone.
        const again = record(captures([]))
        assert.match(again, /nothing moved/)
        assert.equal(JSON.parse(readFileSync(logJson, 'utf8')).entries.length, 0)
    } finally { restore(saved) }
})
