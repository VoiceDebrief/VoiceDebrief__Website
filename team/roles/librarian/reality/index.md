...
| Baselines are updated BY CI and UI change is RECORDED, not blocked (issue 050, supersedes M-qtd-1's gate): `commit-baselines` (qa only) downloads the run's artifact, copies new + changed shots over the baselines, and appends `website/user-guide/baseline-changes.md` + `.json` — each entry naming the shots that moved, by how much, and the **commit + CI run** that moved them. A changed shot passes the run; a pixel threshold cannot tell a redesign from a regression, so an agent reviews the log (issue 051, NOT YET BUILT). Loop-safe: `GITHUB_TOKEN` pushes do not trigger workflows, plus `paths-ignore` on the screenshots path | `.github/workflows/qa-deploy.yml`, `scripts/record_baseline_changes.py`, `tests/unit/baseline-changes.test.mjs` | 2026-08-08 |
