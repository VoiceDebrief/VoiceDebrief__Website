...
| Baselines are armed BY CI (`commit-baselines` job, qa only): downloads the run's `qa-to-docs` artifact and commits **only shots with no existing baseline**, then `deploy-netlify` pulls so they ship in the same run. A shot whose baseline exists and differs is the gate firing — the test job is red, this job never runs, and nothing overwrites the evidence. Loop-safe: `GITHUB_TOKEN` pushes do not trigger workflows (same property the version bump relies on), plus `paths-ignore` on the screenshots path | `.github/workflows/qa-deploy.yml` | 2026-08-07 |
